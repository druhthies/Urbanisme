...
if (!defined('DOSSIER_BASE_PATH')) {
    // Détection fiable de Render (Render injecte RENDER=true automatiquement)
    $isRender = !empty(getenv('RENDER')) || 
                !empty(getenv('RENDER_SERVICE_ID')) ||
    ...
    if ($isRender) {
        ...
        if ($nasPath && @is_dir($nasPath)) {
            ...
            // Pas de NAS accessible = pas de fonctionnalité documents
            define('DOSSIER_BASE_PATH', false);
            define('ENVIRONMENT', 'RENDER-NO-NAS');
            error_log("❌ RENDER - NAS non accessible. DOSSIER_BASE_PATH=" . ($nasPath ?: 'NOT SET'));
        }
    } else {
