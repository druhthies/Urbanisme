...
CREATE TABLE IF NOT EXISTS dossiers (
  ...
  usage VARCHAR(50),
  type_demande VARCHAR(30), -- CONSTRUCTION, EXTENSION, TRANSFORMATION, SURELEVATION, TRANSFORMATION_SURELEVATION
  ...
  superficie_existante DECIMAL(10, 2), -- Pour extension : superficie déjà construite (m²)
  superficie_extension DECIMAL(10, 2), -- Pour extension : superficie de l'extension à construire (m²)
...
-- ===================================================================
-- MIGRATION : nouveaux types de demande (EXTENSION, TRANSFORMATION + SURELEVATION)
-- À exécuter une seule fois sur une base de données déjà existante.
-- (Sans effet si la table dossiers vient d'être créée avec le script ci-dessus.)
-- ===================================================================

-- Élargir la colonne type_demande pour accepter 'TRANSFORMATION_SURELEVATION' (28 caractères)
ALTER TABLE dossiers ALTER COLUMN type_demande TYPE VARCHAR(30);

-- Ajouter les deux nouvelles colonnes utilisées par le type de demande EXTENSION
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS superficie_existante DECIMAL(10, 2);
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS superficie_extension DECIMAL(10, 2);

-- Ajouter les deux nouvelles colonnes utilisées par le type de demande RENOUVELLEMENT
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS num_arrete_origine VARCHAR(50);
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS date_arrete_origine DATE;

-- ===================================================================
-- MIGRATION : informations d'état civil du requérant, nécessaires à la
-- génération de l'Arrêté d'Autorisation de Construire (Article 1er :
-- Né(e) le / à, Profession, Demeurant).
-- À exécuter une seule fois sur une base de données déjà existante.
-- ===================================================================
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS profession VARCHAR(150);
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS date_naissance DATE;
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS lieu_naissance VARCHAR(150);
ALTER TABLE dossiers ADD COLUMN IF NOT EXISTS demeurant VARCHAR(255);
