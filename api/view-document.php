...
$dossiersBasePath = defined('DOSSIER_BASE_PATH') ? DOSSIER_BASE_PATH : false;

if (!$dossiersBasePath) {
    error_log("⚠️ DOSSIER_BASE_PATH non configuré (false). Env: " . (defined('ENVIRONMENT') ? ENVIRONMENT : 'UNKNOWN'));
    ...
    echo '<p style="font-size:12px; text-align:center; color:#ccc; margin-top:30px;">Fonctionnalité documents non disponible sur cet environnement.</p>';
    echo '<p style="font-size:11px; text-align:center; color:#999;">Environnement: ' . (defined('ENVIRONMENT') ? ENVIRONMENT : 'UNKNOWN') . '</p>';
    echo '</body></html>';
    exit;
}

// Vérifier que le chemin est accessible
if (!@is_dir($dossiersBasePath)) {
    error_log("⚠️ NAS non accessible: {$dossiersBasePath}");
    http_response_code(503);
    echo '<html><head><meta charset="utf-8"><style>body { font-family: Arial, sans-serif; padding: 40px; }</style></head><body>';
    echo '<p style="text-align:center; color:#c0392b; font-size:16px;">❌ Serveur NAS indisponible</p>';
    echo '<p style="font-size:12px; text-align:center; color:#ccc; margin-top:30px;">Chemin NAS: ' . htmlspecialchars($dossiersBasePath) . '</p>';
    echo '<p style="font-size:11px; text-align:center; color:#999;">Vérifiez la connexion NAS.</p>';
    echo '</body></html>';
    exit;
}
