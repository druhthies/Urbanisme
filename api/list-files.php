...
if (!$basePath) {
    ...
    echo json_encode(['status' => 503, 'message' => 'Fonctionnalité documents non disponible']);
    exit;
}

if (!@is_dir($basePath)) {
    error_log("⚠️ NAS non accessible: {$basePath}");
    http_response_code(503);
    echo json_encode(['status' => 503, 'message' => 'Serveur NAS indisponible']);
    exit;
}
