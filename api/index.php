...
// ===================================================================
// ACTION DIRECTE: lister_fichiers_dossier (pour l'oeil arrete/avis)
// ===================================================================
$action = isset($_GET['action']) ? $_GET['action'] : '';
if ($action === 'lister_fichiers_dossier') {
    $chemin = isset($_GET['chemin']) ? urldecode($_GET['chemin']) : '';
    if (empty($chemin)) {
        sendJSON(400, 'Chemin manquant');
    }
    // Normaliser en chemins Linux/Synology
    $chemin = str_replace('\\', '/', $chemin);

    $apiPrefix = (strpos($_SERVER['REQUEST_URI'], '/web/') !== false) ? '/web/api/' : '/api/';
    $fichiers = [];
    if (is_dir($chemin)) {
        ...
        foreach ($items as $item) {
            ...
            if (is_file($fullPath)) {
                ...
                $fichiers[] = [
                    'nom' => $item,
                    'url' => $apiPrefix . 'index.php?action=servir_fichier&chemin=' . urlencode($fullPath),
                    'ext' => $ext,
    ...
    header('Content-Type: application/json');
    echo json_encode(['status' => 200, 'fichiers' => $fichiers, 'chemin' => $chemin, 'est_dossier' => is_dir($chemin)]);
    exit;
}

if ($action === 'app_config') {
    sendJSON(200, 'Configuration application', [
        'public_suivi_base_url' => defined('PUBLIC_SUIVI_BASE_URL') ? PUBLIC_SUIVI_BASE_URL : '',
        'isOnNAS' => defined('IS_ON_NAS') ? IS_ON_NAS : false,
    ]);
}

// ===================================================================
// FIABILITE RESEAU: timeouts courts + retry automatique sur les appels
// Supabase. Corrige les pannes intermittentes dues a un DNS/reseau
// instable (au lieu d'attendre 30s puis planter, on echoue vite et on
// retente 2 fois avant d'abandonner).
// ===================================================================
    ...
    function curlExecWithRetry($ch, $maxRetries = 2) {
        ...
        while ($attempt <= $maxRetries) {
            ...
            if (function_exists('logDebug')) {
                logDebug('Retry cURL apres erreur transitoire', [
                    'errno' => $errno,
...
// ===================================================================
// ACTION DIRECTE: lister_fichiers_dossier (pour l'œil arrêté/avis)
// ===================================================================
$action = isset($_GET['action']) ? $_GET['action'] : '';
if ($action === 'lister_fichiers_dossier') {
    $chemin = isset($_GET['chemin']) ? urldecode($_GET['chemin']) : '';
...
// ACTION: servir_fichier
if ($action === 'servir_fichier') {
    $chemin = isset($_GET['chemin']) ? $_GET['chemin'] : '';
    if (empty($chemin) || !file_exists($chemin)) {
        http_response_code(404);
        echo json_encode(['status' => 404, 'message' => 'Fichier introuvable']);
        exit;
    }
...
if ($action === 'log_activite' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $payload = json_decode(file_get_contents('php://input'), true);
    if (!is_array($payload)) {
        sendJSON(400, 'Payload JSON invalide');
    }

    $actionName = strtoupper(trim((string) ($payload['action'] ?? '')));
    $numDossier = trim((string) ($payload['num_dossier'] ?? ''));
    ...
    if ($actionName === '' || $numDossier === '') {
        sendJSON(400, 'action et num_dossier sont requis pour journaliser une activité');
    }

    $lookupUrl = SUPABASE_API_URL . '/dossiers?num_dossier=eq.' . rawurlencode($numDossier) . '&select=id';
    $lookupCh = curl_init($lookupUrl);
    configureCurlDefaults($lookupCh);
    curl_setopt($lookupCh, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($lookupCh, CURLOPT_HTTPHEADER, $SUPABASE_HEADERS);

    $lookupResponse = curlExecWithRetry($lookupCh);
    $lookupHttpCode = curl_getinfo($lookupCh, CURLINFO_HTTP_CODE);
    curl_close($lookupCh);

    if ($lookupHttpCode !== 200) {
        sendJSON(404, 'Dossier introuvable pour la journalisation de consultation');
    }

    $dossierRows = json_decode($lookupResponse, true);
    if (!is_array($dossierRows) || count($dossierRows) === 0) {
        sendJSON(404, 'Dossier introuvable pour la journalisation de consultation');
    }

    $dossierId = (int) ($dossierRows[0]['id'] ?? 0);
    if ($dossierId <= 0) {
        sendJSON(404, 'Dossier introuvable pour la journalisation de consultation');
    }
    ...
    $historyPayload = [
        ...
        'user_id' => $userId,
        'action' => $actionName,
        'field_name' => $detail !== '' ? $detail : 'consultation_suivi_public',
        'new_value' => $numDossier,
        'changed_at' => date('Y-m-d H:i:s')
    ];

    $insertUrl = SUPABASE_API_URL . '/dossier_history';
    $insertCh = curl_init($insertUrl);
    configureCurlDefaults($insertCh);
    curl_setopt($insertCh, CURLOPT_POST, true);
    curl_setopt($insertCh, CURLOPT_POSTFIELDS, json_encode($historyPayload));
    curl_setopt($insertCh, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($insertCh, CURLOPT_HTTPHEADER, $SUPABASE_HEADERS);

    $insertResponse = curlExecWithRetry($insertCh);
    $insertHttpCode = curl_getinfo($insertCh, CURLINFO_HTTP_CODE);
    curl_close($insertCh);

    if ($insertHttpCode === 201) {
        sendJSON(201, 'Activité journalisée', json_decode($insertResponse, true));
    }

    sendJSON($insertHttpCode ?: 500, 'Erreur lors du journal d’activité', ['response' => $insertResponse]);
}
...
if ($method === 'GET') {
    ...
    // GET /api/history - Récupérer l'historique de tous les utilisateurs (admin seulement)
    elseif ($route_name === 'history' && !isset($route[1])) {
    ...
    // GET /api/open-folder?path=... - Ouvrir un dossier physique (NAS uniquement)
    elseif ($route_name === 'open-folder') {
        openFolder();
    }
...
function createDossier() {
    ...
    $isComplete = isset($data['situation_dossier']) && strtoupper($data['situation_dossier']) === 'COMPLETE';
    $isRetraitOui = isset($data['retrait']) && strtoupper($data['retrait']) === 'OUI';
    $shouldArchive = $isComplete && $isRetraitOui;
...
function registerUser() {
    ...
    $data = json_decode(file_get_contents('php://input'), true);
    $requestedRole = strtoupper(trim($data['role'] ?? 'CONSULTANT'));
    $callerRole = strtoupper(trim($_SERVER['HTTP_X_USER_ROLE'] ?? ''));

    // Seuls les admins peuvent créer des comptes ADMIN.
    if ($requestedRole === 'ADMIN' && $callerRole !== 'ADMIN') {
        sendJSON(403, 'Accès refusé : seuls les administrateurs peuvent créer un compte ADMIN');
        return;
    }

    // Les inscriptions publiques peuvent créer des comptes CONSULTANT ou AGENT.
    if (!in_array($requestedRole, ['CONSULTANT', 'AGENT', 'ADMIN'], true)) {
        sendJSON(400, 'Rôle invalide');
        return;
    }
...
function getUsers() {
    ...
    $userRole = isset($_SERVER['HTTP_X_USER_ROLE']) ? strtoupper(trim((string) $_SERVER['HTTP_X_USER_ROLE'])) : null;
    if ($userRole !== 'ADMIN') {
...
function getDbTable() {
    ...
    if ($table === 'dossiers') {
        // Récupérer les dossiers avec leurs avis et les trier pour un affichage cohérent
        $url = SUPABASE_API_URL . '/dossiers?select=*,avis_services(*)&order=ordre.asc,num_dossier.asc';
        $ch = curl_init($url);
        configureCurlDefaults($ch);
            ...
            $transformedData = array_map(function($dossier) {
                ...
                if (isset($dossier['avis_services']) && is_array($dossier['avis_services'])) {
                    foreach ($dossier['avis_services'] as $avis) {
                        $serviceKey = strtolower(trim((string)($avis['service_name'] ?? '')));
                        if ($serviceKey !== '') {
                            $avisMap['avis_' . $serviceKey] = $avis['avis'];
                        }
                    }
                }
...
function saveAvisService() {
    ...
    $serviceName = strtolower(trim((string)$data['service_name']));
    $serviceName = preg_replace('/\s+/', '', $serviceName);
    
    // Vérifier si l'avis existe déjà pour ce dossier et service
    $checkUrl = SUPABASE_API_URL . '/avis_services?dossier_id=eq.' . (int)$data['dossier_id'] . '&service_name=eq.' . rawurlencode($serviceName) . "&select=*";
    ...
    $checkResponse = curlExecWithRetry($ch);
    $checkHttpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
...
function getAllUsersWithStats() {
    ...
    // Vérifier que seul un admin peut voir les statistiques des utilisateurs
    $userRole = isset($_SERVER['HTTP_X_USER_ROLE']) ? strtoupper(trim((string) $_SERVER['HTTP_X_USER_ROLE'])) : null;
    if ($userRole !== 'ADMIN') {
        sendJSON(403, 'Accès refusé : seuls les administrateurs peuvent voir les statistiques des utilisateurs');
        return;
    }
    ...
    if ($httpCode === 200) {
        ...
        // Pour chaque utilisateur, récupérer le nombre de dossiers modifiés et la dernière activité
        foreach ($users as &$user) {
            $historyUrl = SUPABASE_API_URL . '/dossier_history?user_id=eq.' . urlencode($user['id']) . '&select=count()';
            $histCh = curl_init($historyUrl);
            configureCurlDefaults($histCh);
            curl_setopt($histCh, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($histCh, CURLOPT_HTTPHEADER, array_merge($SUPABASE_HEADERS, ['Prefer: count=exact']));
            curlExecWithRetry($histCh);
            $dossiersCount = curl_getinfo($histCh, CURLINFO_HTTP_HEADER_OUT);
            curl_close($histCh);
            
            $user['dossiers_count'] = 0;
            $user['last_activity'] = '-';
        }
        
        sendJSON(200, 'Utilisateurs avec statistiques récupérés', $users);
    } else {
        sendJSON($httpCode, 'Erreur Supabase', ['response' => $response]);
    }
}
...
function getAllUserActivities() {
    ...
    // Vérifier que seul un admin peut voir l'historique complet
    $userRole = isset($_SERVER['HTTP_X_USER_ROLE']) ? strtoupper(trim((string) $_SERVER['HTTP_X_USER_ROLE'])) : null;
    if ($userRole !== 'ADMIN') {
        sendJSON(403, 'Accès refusé : seuls les administrateurs peuvent voir l\'historique complet');
        return;
    }
    
    $url = SUPABASE_API_URL . '/dossier_history?select=*&order=changed_at.desc&limit=500';
    ...
    if ($httpCode === 200) {
        $history = json_decode($response, true);
        sendJSON(200, 'Historique complet récupéré', $history);
    } else {
        sendJSON($httpCode, 'Erreur: ' . $response);
    }
}

function tryOpenFolderWithSystemCommand($path) {
    $normalizedPath = str_replace('\\', '/', $path);
    $osFamily = defined('PHP_OS_FAMILY') ? PHP_OS_FAMILY : PHP_OS;

    if ($osFamily === 'Windows') {
        $commands = [
            'explorer.exe "' . str_replace('"', '""', $normalizedPath) . '"',
            'powershell -NoProfile -Command Start-Process -FilePath "' . str_replace('"', '""', $normalizedPath) . '"'
        ];
    } elseif ($osFamily === 'Linux') {
        $commands = [
            'xdg-open "' . str_replace('"', '""', $normalizedPath) . '"',
            'gio open "' . str_replace('"', '""', $normalizedPath) . '"'
        ];
    } elseif ($osFamily === 'Darwin') {
        $commands = ['open "' . str_replace('"', '""', $normalizedPath) . '"'];
    } else {
        $commands = [];
    }
...
// Ouvrir un dossier physique sur le NAS ou sur la machine hôte
function openFolder() {
    $path = isset($_GET['path']) ? $_GET['path'] : '';
    
    if (empty($path)) {
        sendJSON(400, 'Paramètre path manquant');
        return;
    }
    
    // Sécurité : vérifier que le chemin est dans le NAS autorisé
    $basePath = str_replace('\\', '/', DOSSIER_BASE_PATH);
    $normalizedPath = str_replace('\\', '/', $path);
    
    if (strpos($normalizedPath, $basePath) !== 0) {
        sendJSON(403, 'Chemin non autorisé : doit être dans le dossier NAS', [
            'path' => $normalizedPath,
            'basePath' => $basePath
        ]);
        return;
    ...
    if (is_dir($normalizedPath)) {
        $openResult = tryOpenFolderWithSystemCommand($normalizedPath);
        sendJSON(200, 'Dossier accessible', [
            'path' => $normalizedPath,
            'exists' => true,
            'opened' => $openResult['ok'],
            'isOnNAS' => IS_ON_NAS,
            'info' => $openResult['ok'] ? 'Dossier ouvert' : 'Ouverture automatique impossible, utilisez l\'Explorateur Windows avec ce chemin : ' . $normalizedPath
        ]);
    } else {
        sendJSON(404, 'Dossier introuvable sur le NAS', [
            'path' => $normalizedPath,
            'exists' => false,
            'isOnNAS' => IS_ON_NAS,
            'basePath' => $basePath
