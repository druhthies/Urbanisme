...
function updateDossier($id) {
    ...
    $input = file_get_contents('php://input');
    logDebug("PUT dossier", ['id' => $id, 'input_length' => strlen($input)]);
    
    $data = json_decode($input, true);
    
    if (!$data || !is_array($data)) {
        logDebug("Invalid JSON", ['input' => substr($input, 0, 100), 'json_error' => json_last_error_msg()]);
        sendJSON(400, 'Données JSON invalides');
        return;
    }
...
function deleteUser($id) {
    ...
    logDebug("deleteUser called", ['id' => $id]);
    
    // Vérifier que seul un admin peut supprimer un utilisateur
    $userRole = isset($_SERVER['HTTP_X_USER_ROLE']) ? $_SERVER['HTTP_X_USER_ROLE'] : null;
    logDebug("User role check", ['userRole' => $userRole]);
    ...
    logDebug("Deleting user", ['url' => $url]);
    ...
    logDebug("Delete response", ['httpCode' => $httpCode, 'response' => $response]);
    
    // Accepter 200 (OK) ou 204 (No Content) comme succès
    if ($httpCode === 200 || $httpCode === 204) {
        sendJSON(200, 'Utilisateur supprimé avec succès');
    } else {
        sendJSON($httpCode, 'Erreur Supabase: ' . $response, ['httpCode' => $httpCode]);
    }
}
